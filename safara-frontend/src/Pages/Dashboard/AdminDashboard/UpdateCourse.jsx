...
function UpdateCourse() {
    ...
    const fetchCourseData = async () => {
      try {
        ...
        if (response.ok) {
          ...
          setEnabledPaymentMethods(courseData.paymentMethods || ["online", "manual"]);
          setQuizzes(
  ...
  const handleAddVideo = (e) => {
    ...
    if (videoFile) {
      setSelectedVideos([
        ...selectedVideos,
        { videoTitle: videoFile.name, videoFile, videoType: "upload" },
      ]);
    }
  };
    ...
    try {
      ...
      for (const video of selectedVideos) {
        if (video.videoType === "youtube") {
          ...
          // Preserve existing video data from DB (e.g. when updating other fields)
    ...
    <div>
      {loading ? (
        ...
        <form onSubmit={handleSubmit} className="lg:p-6 pt-6">
          ...
          {/* NEW FEATURE: Payment methods toggle */}
          <div className="flex gap-4 mt-3">
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                type="checkbox"
                checked={enabledPaymentMethods.includes("online")}
                onChange={(e) => {
                  if (e.target.checked) {
                    setEnabledPaymentMethods([...enabledPaymentMethods, "online"]);
                  } else {
                    setEnabledPaymentMethods(enabledPaymentMethods.filter((m) => m !== "online"));
                  }
                }}
            ...
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                type="checkbox"
                checked={enabledPaymentMethods.includes("manual")}
                onChange={(e) => {
                  if (e.target.checked) {
                    setEnabledPaymentMethods([...enabledPaymentMethods, "manual"]);
                  } else {
                    setEnabledPaymentMethods(enabledPaymentMethods.filter((m) => m !== "manual"));
                  }
                }}
                className="checkbox checkbox-primary checkbox-sm"
              />
              <span className="text-sm font-medium">Manual Payment (bKash / Nagad / Rocket / Bank)</span>
            </label>
          </div>
          ...
          <div className="grid md:grid-cols-2 grid-cols-1 gap-3">
            <div className="grid grid-cols-4 gap-3">
              <div className="form-control col-span-4 w-full">
                <label className="label">
                  <span className="label-text">Upload Video File</span>
                </label>
                <input
                  type="file"
              ...
              <div className="form-control col-span-4 w-full">
                <label className="label">
                  <span className="label-text">Or Add YouTube Link</span>
                </label>
                <div className="flex flex-col gap-2">
                  <input
                    ...
                    placeholder={`Lecture title (default: Lecture ${selectedVideos.length + 1})`}
                    className="px-3 py-[11px] rounded-md border border-slate-200 w-full"
                  />
                  <div className="flex gap-2">
                    ...
                    <p
                      ...
                      className="border h-fit text-center rounded-md py-[11px] px-4 cursor-pointer bg-slate-200 whitespace-nowrap"
                    >
                      Add YouTube
                    </p>
                  </div>
                  ...
                  >
                    <div className="flex gap-3 items-center">
                      ...
                      {video.videoType === "youtube" && (
                        <span className="badge badge-error text-white text-xs">YouTube</span>
                      )}
                      {(!video.videoType || video.videoType === "upload") && (
                        <span className="badge badge-primary text-white text-xs">Upload</span>
                      )}
                      <p>
                        {video.videoTitle.slice(0, 16)}......
                      </p>
                    </div>
                    <RxCross2
